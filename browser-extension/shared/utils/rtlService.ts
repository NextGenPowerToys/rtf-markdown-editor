/**
 * RTL Service for Browser Extension
...
export class RTLService {
  ...
  static applyToDocument(rtl: boolean): void {
    const direction = RTLService.getDirection(rtl);
    ...
    // Apply to ProseMirror content
    const prosemirror = document.querySelector('.ProseMirror') as HTMLElement;
    if (prosemirror) {
      prosemirror.style.direction = direction;
      prosemirror.style.textAlign = rtl ? 'right' : 'left';
    }
  }
  ...
  static hasRTLContent(jsonContent: any): boolean {
    ...
    return RTLService.RTL_CHAR_PATTERN.test(jsonString);
  }
  ...
  static applyConfig(config: RTLConfig): void {
    RTLService.applyToDocument(config.rtl);
    RTLService.updateButtonUI(config.rtl);
  }
}
